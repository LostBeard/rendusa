...
THREE.RendusaRenderers['2DZ'] = function ( renderer, source, target, _rendusa ) {
	...
	function initInterleaver(_outFormatReq){
		...
		shader.uniforms[ "last_view" ].value = interleaver.views - 1;
	...
	$(target.target_el).on('rendusa_ui_pos_pre', function(e, pos){
		...
		if (oThis.active) {
			if (oThis.outFormat == 2 || oThis.outFormat == 3){
				pos.x *= 2;
			}
		}
	});
	...
	this.render = function ( ) {
		var _outFormatName = oThis.outFormats[oThis.outFormat];
		...
		else if (this.outFormat == 1)
		{
			 // anaglyph
			merge.uniforms[ "outFormat" ].value = oThis.outFormat;
			merge.uniforms[ "redMixer" ].value = anaglyph.mixer.red;
			merge.uniforms[ "greenMixer" ].value = anaglyph.mixer.green;
			merge.uniforms[ "blueMixer" ].value = anaglyph.mixer.blue;
			merge.uniforms[ "viewTexArray" ].value = renderTargetViewsTextures();
		} 
		else if (this.outFormat == 5)
		{
