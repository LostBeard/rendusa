...
THREE.RendusaRenderers['RendusaUI'] = function ( renderer, source, target, _rendusa ) {
    ...
    $('<tr>').appendTo($info_table).append('<td><div>Media</div></td>').append('<td><div class="rendusa_ui_in_src_name_label"></div></td>');
    ...
    var update_info = function(){
        var format3D = source.info.format3D === '2DZ' ? '2D+Z' : format3D;
        var src_name = source.info.filename ? source.info.filename : '';
        var game = source.info.steam_name ? source.info.steam_name : '';
        var shared_by = source.info.shared_by ? source.info.shared_by : '';
        //
        ...
        $ui.find('.rendusa_ui_in_format_3d_label').text(format3D);
