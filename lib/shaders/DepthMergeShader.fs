...
vec4 anaglyph_mixer(vec4 l, vec4 r) {
...
void main() {
      vec4 colorM, colorL, colorR;
      vec2 uv = vUv;
      if (outFormat == 0 || outFormat == 4) {
            // 2D or Perspective
            colorR = texture2D( viewTexArray[0], uv );
            gl_FragColor = vec4( colorR.r, colorR.g, colorR.b, 1.0 );
      //} else if (outFormat == 1) {
            // // Anaglyph
            // colorL = texture2D( viewTexArray[0], uv );
            // colorR = texture2D( viewTexArray[1], uv );
            // gl_FragColor = anaglyph_mixer(colorL, colorR) * brightness;
      } else if (outFormat == 2) {
            // Stereo Side By Side
            if (uv.x < 0.5) {
                  uv.x = uv.x * 2.0;
                  colorM = texture2D( viewTexArray[0], uv );
                  gl_FragColor = vec4( colorM.r, colorM.g, colorM.b, colorM.a);
            } else {
                  uv.x = (uv.x - 0.5) * 2.0;
                  colorM = texture2D( viewTexArray[1], uv );
                  gl_FragColor = vec4( colorM.r, colorM.g, colorM.b, colorM.a);
            }
      } else if (outFormat == 3) {
            // 2DZ
