...
THREE.Rendusa.prototype = {
	...
	_setSrcImage: function(srcInfo){
		...
		$(this.target.target_el).trigger('rendusa_source_change_start');
		var srcid = this.source.srcID;
		this._textureLoader.load(
			srcInfo.url,
			function ( texture ) {
				// succcess
				if (srcid == oThis.source.srcID) oThis._load_complete(true, texture);
			},
			function ( xhr ) {
				// progress
				...
				if (srcid == oThis.source.srcID) oThis.source.durationDownloaded = oThis.source.duration * perc;
			},
			function ( xhr ) {
				// error
				if (srcid == oThis.source.srcID) oThis._load_complete(false, null);
			}
		);
	},
